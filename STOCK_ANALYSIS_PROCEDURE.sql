...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id('CREATE_DRV_THEME_EMBEDDING_SET') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE CREATE_DRV_THEME_EMBEDDING_SET
...
GO
CREATE PROCEDURE CREATE_DRV_THEME_EMBEDDING_SET
AS
BEGIN
	IF EXISTS (SELECT * FROM sysobjects WHERE name='THEME_EMBEDDING_SET' AND xtype='U')
		DROP TABLE THEME_EMBEDDING_SET

	-- 테마별로 주식들을 평균으로 묶는다. 상승이 5퍼센트 이상인 경우만 남김.
	SELECT DISTINCT B.THEME_CODE, AVG(A.AVERAGE) AVERAGE, AVG(A.VOLUME) VOLUME, AVG(A.CHANGE_RATIO) CHANGE_RATIO, A.DATE
	INTO #DATE_THEME_AVERAGE
	FROM STOCK_PRICE_RATIO A
	RIGHT JOIN STOCK_THEME B ON A.STOCK_CODE = B.STOCK_CODE 
	WHERE CHANGE_RATIO > 1.05 
	GROUP BY B.THEME_CODE, A.[DATE]
	
	SELECT A.THEME_CODE BEF_THEME, B.THEME_CODE AFT_THEME
	INTO THEME_EMBEDDING_SET
	FROM #DATE_THEME_AVERAGE A
	JOIN #DATE_THEME_AVERAGE B ON A.[DATE] = DATEADD (DAY , -1, B.[DATE])
...
BEGIN
	...
	EXEC CREATE_DRV_THEME_EMBEDDING_SET
END
GO
