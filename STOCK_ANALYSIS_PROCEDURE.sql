...
BEGIN
	...
	ORDER BY STOCK_CODE, [YEAR] DESC, [WEEK] DESC 

	SELECT *, 
		LEAD([YEAR]) OVER (ORDER BY A.STOCK_CODE, [YEAR] DESC, [WEEK] DESC) AS BEF_YEAR,
		LEAD([WEEK]) OVER (ORDER BY A.STOCK_CODE, [YEAR] DESC, [WEEK] DESC) AS BEF_WEEK,
		CONVERT(FLOAT, A.[AVERAGE]) / LEAD([AVERAGE]) OVER (ORDER BY A.STOCK_CODE, [YEAR] DESC, [WEEK] DESC) AS CHANGE_RATE
	INTO DRV_STOCK_PRICE_WEEK
	FROM #STOCK_PRICE_WEEK A
	ORDER BY A.STOCK_CODE, A.[YEAR] DESC, A.[WEEK] DESC

	DELETE FROM DRV_STOCK_PRICE_WEEK WHERE BEF_YEAR > [YEAR]
...
AS
BEGIN
	SELECT A.STOCK_CODE, A.AVERAGE, A.HIGHEST, A.LOWEST, A.VOLUME, A.CHANGE_RATIO, A.WEEK_AVG, A.WEEK_RATIO, A.MARKET_CAP,
