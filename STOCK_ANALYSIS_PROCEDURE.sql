...
AS
BEGIN
	IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='STOCK_EMBEDDING_WEIGHTS' AND xtype='U')
	BEGIN
...
BEGIN
	...
	-- 주가 상승이 5퍼센트 이상인 경우만 남김.
	SELECT A.STOCK_CODE, A.AVERAGE, A.VOLUME, A.CHANGE_RATIO, A.DATE
	INTO #DATE_THEME_AVERAGE
	FROM STOCK_TRAINING_SET A
	WHERE CHANGE_RATIO > 1.05
	
	-- 해당 주식들 중 같은날 5퍼센트 이상 상승한 주식과 연결
	SELECT A.STOCK_CODE BEF_STOCK, B.STOCK_CODE AFT_STOCK
	INTO STOCK_EMBEDDING_SET
	FROM #DATE_THEME_AVERAGE A
	JOIN #DATE_THEME_AVERAGE B ON A.[DATE] = B.[DATE]

	-- 학습을 위해 False Set 생성
	SELECT DISTINCT STOCK_CODE 
	INTO #STOCK_LIST
	FROM STOCK_TRAINING_SET

	SELECT A.STOCK_CODE BEF_STOCK, B.STOCK_CODE AFT_STOCK
	INTO STOCK_FALSE_SET
	FROM #STOCK_LIST A
	CROSS JOIN #STOCK_LIST B
...
GO
CREATE PROCEDURE SEL_STOCK_TEST_DATA @STOCK_CODE INT, @DATE DATE
AS
BEGIN
	SELECT A.STOCK_CODE, A.AVERAGE, A.HIGHEST, A.LOWEST, A.VOLUME, A.CHANGE_RATIO, A.MARKET_CAP,
		A.PBR, A.PBR2, A.PDR, A.PER, A.PCR_OP, A.PCR_IV, A.PCR_FI, A.PSR, A.PFR, A.PXR, A.ROE, A.ROA, A.MARKET_RANK, A.[DATE]
	FROM [STOCK_ANALYSIS].[dbo].[STOCK_TRAINING_SET] A
    WHERE STOCK_CODE = @STOCK_CODE AND [DATE] BETWEEN DATEADD(WEEK, -1, @DATE) AND @DATE
    ORDER BY DATE
END

GO	
CREATE PROCEDURE SEL_STOCK_TEST_DATA_WEEK @STOCK_CODE INT, @DATE DATE
AS
BEGIN
	SELECT A.STOCK_CODE, A.AVERAGE, A.HIGHEST, A.LOWEST, A.VOLUME, A.CHANGE_RATIO, A.MARKET_CAP,
		A.PBR, A.PBR2, A.PDR, A.PER, A.PCR_OP, A.PCR_IV, A.PCR_FI, A.PSR, A.PFR, A.PXR, A.ROE, A.ROA, A.MARKET_RANK, A.[DATE]
	FROM [STOCK_ANALYSIS].[dbo].[STOCK_TRAINING_SET_WEEK] A
    WHERE STOCK_CODE = @STOCK_CODE AND [DATE] BETWEEN DATEADD(MONTH, -1, @DATE) AND @DATE
    ORDER BY DATE
END
